...
@pytest.mark.parametrize(
    ...
    [(OperationType.MAX, 'age', 'movie_name', {'inception': 44, 'pulp_fiction': 44, "ender's_game": 0}),
     (OperationType.MAX, 'score', 'movie_name', {'inception': 8, 'pulp_fiction': 8, "ender's_game": 8}),
     (OperationType.MAX, 'score', 'first_name', {'tim': 8, 'tamas': 7, 'dave': 8}),
     (OperationType.MIN, 'age', 'movie_name', {'inception': 0, 'pulp_fiction': 26, "ender's_game": 0}),
     (OperationType.MIN, 'score', 'movie_name', {'inception': 7, 'pulp_fiction': 4, "ender's_game": 8}),
     (OperationType.MIN, 'score', 'first_name', {'tim': 8, 'tamas': 4, 'dave': 8}),
     (OperationType.AVG, 'age', 'movie_name', {'inception': 70 / 3, 'pulp_fiction': 35, "ender's_game": 0}),
     (OperationType.AVG, 'score', 'movie_name', {'inception': 7.666666666666667, 'pulp_fiction': 6, "ender's_game": 8}),
     (OperationType.AVG, 'score', 'first_name', {'tim': 8, 'tamas': 5.5, 'dave': 8}),
     (OperationType.AVG, 'wrong', 'first_name', {}),
     (OperationType.AVG, 'score', 'wrong', {})])
def test_operation(operation_type, aggreagete_column, grouping_column, expected_results) -> None:
    operation = prepare_operation()
    query = Query(operation_type,
